...
class TestComputeInstability:
    ...
    def test_all_metrics_present(self):
        """All 15 keys present in output."""
        reads = [_ri(90.0, 1)] * 10 + [_ri(110.0, 2)] * 10
        result = compute_instability(reads, 100.0, motif_len=3)
        expected_keys = {
            "median_h1", "median_h2", "hii_h1", "hii_h2",
            "ias", "range_h1", "range_h2", "n_h1", "n_h2",
            "n_total", "concordance",
...
class TestWriteInstabilityTsv:
    def test_header_columns(self, tmp_path):
        ...
        assert len(cols) == 20
        assert cols[0] == "#chrom"
        assert cols[-1] == "n_trimmed_h2"
        assert cols[-2] == "n_trimmed_h1"
        assert cols[-3] == "dropout_flag"

    def test_data_row(self, tmp_path):
        """Data row has 20 columns with correct values."""
        output = str(tmp_path / "out.tsv")
        ...
        assert fields[15] == "hp-tagged"
        assert fields[16] == "none"
        assert fields[17] == "0"
        assert fields[18] == "0"  # n_trimmed_h1
        assert fields[19] == "0"  # n_trimmed_h2
    ...
    def test_none_result(self, tmp_path):
        ...
        assert fields[15] == "failed"  # analysis_path
        assert fields[16] == "."  # unstable_haplotype
        assert fields[17] == "0"  # dropout_flag
        assert fields[18] == "0"  # n_trimmed_h1
        assert fields[19] == "0"  # n_trimmed_h2
    ...
    def test_nan_inf_values_produce_dots(self, tmp_path):
        ...
        results = [{
            ...
            "range_h1": 0.0, "range_h2": 0.0,
            "n_h1": 10, "n_h2": 10,
            "n_total": 20, "concordance": float("nan"),
            "analysis_path": "hp-tagged",
            "unstable_haplotype": "none",
            "dropout_flag": False,
            "n_trimmed_h1": 0, "n_trimmed_h2": 0,
...
class TestUnstableHaplotype:
    ...
    def test_pooled_stable_none(self):
        """Pooled fallback with stable reads -> 'none'."""
        reads = [_ri(100.0, 0)] * 10
        result = compute_instability(reads, 100.0, motif_len=3)
        assert result["unstable_haplotype"] == "none"

    def test_pooled_unstable_h1(self):
        """Pooled fallback with unstable reads -> 'h1'."""
        # Spread reads so HII > 0.45 (3σ noise threshold)
        reads = [_ri(90.0 + i * 3.0, 0) for i in range(10)]
        result = compute_instability(reads, 100.0, motif_len=3)
        assert result["hii_h1"] > 0.45
        assert result["unstable_haplotype"] == "h1"

    def test_both_unstable(self):
        """Both haplotypes have high HII -> 'both'."""
        hp1_reads = [_ri(80.0 + i * 3.0, 1) for i in range(10)]
        hp2_reads = [_ri(110.0 + i * 3.0, 2) for i in range(10)]
        result = compute_instability(hp1_reads + hp2_reads, 100.0, motif_len=3)
        assert result["hii_h1"] > 0.45
        assert result["hii_h2"] > 0.45
        assert result["unstable_haplotype"] == "both"
...
class TestDropoutFlag:
    def test_dropout_suspected(self):
        ...
        # 15 reads all at 200bp, ref_size=200bp, motif_len=3
        # size_range=0 < 6, ref_size=200 > 150 (motif*50), IQR=0 < 3 -> dropout
        reads = [_ri(200.0, 0)] * 15
        assert _detect_dropout(reads, ref_size=200.0, motif_len=3) is True

    def test_no_dropout_small_locus(self):
        """Small locus (ref_size <= 50*motif_len) -> no dropout."""
        reads = [_ri(10.0, 0)] * 15
        assert _detect_dropout(reads, ref_size=20.0, motif_len=3) is False

    def test_no_dropout_medium_locus(self):
        """Medium locus (ref_size=50, <= 150 for motif=3) -> no dropout."""
        reads = [_ri(50.0, 0)] * 15
        assert _detect_dropout(reads, ref_size=50.0, motif_len=3) is False

    def test_no_dropout_bimodal(self):
        """Bimodal reads -> size_range >= 2*motif_len -> no dropout."""
        reads = [_ri(200.0, 0)] * 8 + [_ri(210.0, 0)] * 7
        assert _detect_dropout(reads, ref_size=200.0, motif_len=3) is False
    ...
    def test_dropout_in_compute_instability(self):
        ...
        reads = [_ri(200.0, 0)] * 15
        result = compute_instability(reads, ref_size=200.0, motif_len=3)
        assert result is not None
