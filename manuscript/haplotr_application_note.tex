...
\title{MosaicTR: haplotype-aware tandem repeat genotyping from long-read sequencing}
...
We present MosaicTR, a tool that leverages haplotype (HP) tags from phased BAM files to separate alleles prior to sizing, using concordance-based zygosity determination.
Benchmarked against the Genome in a Bottle Tier1 truth set (1.64 million loci), MosaicTR achieves 97.2\% exact allele match, 99.3\% zygosity accuracy, and 99.7\% genotype concordance, outperforming LongTR (87.5\% exact, 99.6\% zygosity) in allele sizing while maintaining comparable zygosity determination.
MosaicTR further provides per-locus confidence scores, enabling users to filter unreliable calls and achieve $>$99.8\% zygosity accuracy on high-confidence loci.
...
\\[6pt]
\textbf{Availability:}
MosaicTR is available at \url{https://github.com/xxx/mosaictr} under the MIT license.
\\[6pt]
...
MosaicTR exploits these existing HP tags to separate reads into haplotype clusters before computing allele sizes, providing a natural framework for diploid genotyping that avoids de novo haplotype inference at each locus.
...
MosaicTR takes an HP-tagged BAM and a TR loci catalog as input.
...
For short tandem repeats (STRs, motif $<$7\,bp), MosaicTR computes MapQ-weighted medians per haplotype with integer rounding.
...
MosaicTR uses a concordance-based zygosity decision.
...
MosaicTR outputs a per-locus confidence score (0--1) combining concordance and sizing quality, enabling downstream filtering.

\subsection*{Benchmarking}

We evaluated MosaicTR against the GIAB Tandem Repeat Benchmark v1.0.1 Tier1 truth set for HG002, which provides assembly-derived allele sizes for 1,638,106 loci genome-wide \citep{wagner2024}.
The Adotto TR catalog v1.2 \citep{dolzhenko2024} defined the loci set.
We compared MosaicTR with LongTR v1.2 \citep{ziaei-jam2024} and TRGT v1.0 \citep{dolzhenko2024} using the same truth set.
...
MosaicTR genotyped all 1,784,804 catalog loci in HG002 with a median of 49 reads per locus.
...
\midrule
MosaicTR v4 & 1,637,425 & 97.2\% & 99.3\% & 0.19 & 99.3\% & 99.7\% \\
LongTR     & 1,623,440 & 87.5\% & 99.1\% & 0.23 & 99.6\% & 99.6\% \\
\bottomrule
\end{tabular}
\end{table}

MosaicTR achieved the highest exact allele match (97.2\% vs 87.5\% for LongTR) and lowest MAE (0.19\,bp vs 0.23\,bp), indicating more precise allele sizing from the HP-weighted median approach.
...
MosaicTR achieved lower MAE than LongTR across all STR categories (motif 1--6\,bp), with both tools achieving sub-0.25\,bp MAE.
For VNTRs (motif $\geq$7\,bp), the two tools showed comparable MAE (0.31 vs 0.28\,bp); MosaicTR's two-pass MAD trimming and gap-based bimodality detection maintained competitive accuracy despite alignment ambiguity in long, complex repeats.

\subsection*{Confidence-based filtering}

MosaicTR's confidence score enables accuracy--yield trade-offs (Figure~\ref{fig:main}c).
...
This feature is unique to MosaicTR among the tools compared.

\subsection*{Mendelian inheritance}

In the HG002/HG003/HG004 trio, MosaicTR achieved 89.6\% strict and 99.3\% off-by-one-motif-unit Mendelian consistency across 1,769,186 common loci.
Published rates are 86\% (LongTR, strict), 98.4\% (TRGT, off-by-one-unit), and 97.9\%/99.1\% (STRkit, strict/relaxed).
MosaicTR's strict rate exceeds LongTR and its relaxed rate exceeds all three published tools, confirming accurate haplotype-aware genotyping genome-wide.

\subsection*{Disease-associated loci}

Of 75 STRchive disease-associated loci in the catalog, 69 (92\%) were genotyped by MosaicTR, compared with 66 each for LongTR and TRGT.
...
\caption{MosaicTR genome-wide benchmarking results.
(a)~Overall genotyping accuracy for MosaicTR and LongTR against GIAB Tier1 truth.
...
MosaicTR demonstrates that leveraging existing haplotype phase information from long-read BAMs provides an effective approach to diploid TR genotyping.
MosaicTR's exact allele match of 97.2\% substantially exceeds LongTR's 87.5\%, while maintaining comparable zygosity accuracy (99.3\% vs 99.6\%), demonstrating that HP-tag--based allele sizing provides superior precision.

Key advantages of MosaicTR include:
...
Future work will extend MosaicTR to population-scale analysis and targeted clinical panels for TR expansion disorders.
