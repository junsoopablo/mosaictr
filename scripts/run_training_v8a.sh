#!/bin/bash
# DeepTR v8A — Bug-Fix Baseline
#
# Hypothesis: CosineAnnealingWarmRestarts LR resets + BCEWithLogitsLoss
#   double-transform were the primary causes of failure in v6/v7.
#   Fix bugs, use ReduceLROnPlateau, conservative reweighting.
#
# Changes vs v7:
#   --scheduler reduce_plateau: stable LR (no warm restarts)
#   --variant-pos-weight 0: bypass BCEWithLogitsLoss bug path entirely
#   --tp-loss-weight 5: conservative TP reweighting
#   --tp-sample-weight 5: TP oversampling
#   --focal-gamma 1.0: reduced from 2.0
#   --patience 10: more patience for convergence
#
# Success criterion: TP exact >= 30%, TN exact >= 90%
set -euo pipefail
...
OUTDIR="${PROJECT}/output/train_v8a"
...
TRAIN_JOB=$(sbatch --parsable \
    --job-name="deeptr_v8a" \
    --output="${SLURMDIR}/deeptr_v8a_%j.out" \
    --error="${SLURMDIR}/deeptr_v8a_%j.err" \
    --time=02:00:00 \
    --mem=64G \
    --cpus-per-task=8 \
    --gpus=l40s:1 \
    --wrap="
        ...
        echo '=== DeepTR v8A: Bug-Fix Baseline ==='
        echo \"Date: \$(date)\"
        echo \"Host: \$(hostname)\"
        echo 'scheduler=reduce_plateau, variant-pos-weight=0'
...
echo "v8A bug-fix baseline job submitted: ${TRAIN_JOB}"
echo ""
echo "Monitor:"
echo "  squeue -u junsoopablo"
echo "  tail -f ${SLURMDIR}/deeptr_v8a_${TRAIN_JOB}.out"
