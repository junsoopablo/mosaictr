#!/bin/bash
# DeepTR v3 training — uses pre-computed features, only retrains model
# Adds per-sample allele loss weighting for TP loci (tp_loss_weight=2.0)
set -euo pipefail
...
OUTDIR="${PROJECT}/output/train_v3"
...
JOBID=$(sbatch --parsable \
    --job-name="deeptr_v3" \
    --output="${SLURMDIR}/deeptr_v3_%j.out" \
    --error="${SLURMDIR}/deeptr_v3_%j.err" \
    --time=02:00:00 \
    --mem=64G \
    --cpus-per-task=8 \
    --gpus=l40s:1 \
    --wrap="
        ...
        echo '=== Starting DeepTR v3 training ==='
        echo \"Date: \$(date)\"
        echo \"Host: \$(hostname)\"
