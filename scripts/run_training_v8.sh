#!/bin/bash
# DeepTR v8 — Step C: Attention Pooling + Scale-Aware Loss
#
# Hypothesis: mean+max pooling loses het variant bimodal read distribution.
#   Scale-aware loss compensates for log-space compression at large variants.
#
# Changes vs Step B:
#   --model-version v2: DeepTRv2 with attention pooling (2 learnable queries)
#   --scale-aware-loss: weight TP allele loss by target magnitude
#
# Success criterion: TP exact match >= 55% (ideally 60%+)
...
OUTDIR="${PROJECT}/output/train_v8"
...
# ---------------------------------------------------------------------------
# Training — Step B + DeepTRv2 + scale-aware loss
# ---------------------------------------------------------------------------
TRAIN_JOB=$(sbatch --parsable \
    --job-name="deeptr_v8_stepC" \
    --output="${SLURMDIR}/deeptr_v8_stepC_%j.out" \
    --error="${SLURMDIR}/deeptr_v8_stepC_%j.err" \
    --time=02:00:00 \
    --mem=64G \
    --cpus-per-task=8 \
    --gpus=l40s:1 \
    --wrap="
        ...
        echo '=== DeepTR v8: Step C — Attention Pooling + Scale-Aware Loss ==='
        echo \"Date: \$(date)\"
        echo \"Host: \$(hostname)\"
        echo 'model-version=v2, scale-aware-loss, tp-only-allele-loss'
        echo 'variant-pos-weight=15.0, tp-batch-fraction=0.3'
        ...
        # Also evaluate without soft-gating
...
echo "Step C training job submitted: ${TRAIN_JOB}"
echo ""
echo "Monitor:"
echo "  squeue -u junsoopablo"
echo "  tail -f ${SLURMDIR}/deeptr_v8_stepC_${TRAIN_JOB}.out"
