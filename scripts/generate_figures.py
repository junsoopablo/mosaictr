#!/usr/bin/env python3
"""Generate publication figures for HaploTR Application Note.

Figure 1 (main, multi-panel):
  (a) Tool accuracy bar chart (Exact, <=1bp, GenConc) — 3 tools
  (b) Motif period vs MAE grouped bar
  (c) Confidence threshold vs accuracy curve (HaploTR only)
...
from haplotr.benchmark import load_predictions, _motif_period_bin, _repeat_length_bin
...
def fig1_motif_mae(ax, ht_pairs, lt_pairs, trgt_pairs=None):
    ...
    ht_by = stratify_pairs(ht_pairs, haplotr_motif_bin)
    lt_by = stratify_pairs(lt_pairs, tool_motif_bin)

    x = np.arange(len(motif_order))
    tool_list = [
        ("HaploTR", ht_by, compute_haplotr_metrics, COLORS["HaploTR"]),
...
def fig1_confidence_curve(ax, ht_pairs):
    ...
    ax.plot(thresholds, geno_concs, "o-", color=COLORS["HaploTR"],
...
def fig2_mendelian(ax, mendelian_data=None):
    ...
    # Published values + placeholder for HaploTR
    tools = ["LongTR", "TRGT", "STRkit", "HaploTR"]
...
def fig2_delta_accuracy(ax, ht_pairs, lt_pairs, trgt_pairs=None):
    ...
    for tool_name, pairs, compute_fn, color, is_ht in [
        ("HaploTR", ht_pairs, compute_haplotr_metrics, COLORS["HaploTR"], True),
...
def fig2_length_accuracy(ax, ht_pairs, lt_pairs, trgt_pairs=None):
    ...
    for tool_name, pairs, compute_fn, color, bin_fn in [
        ("HaploTR", ht_pairs, compute_haplotr_metrics, COLORS["HaploTR"], haplotr_length_bin),
...
def main():
    ...
    parser.add_argument("--mendelian-strict", type=float, default=None,
                        help="HaploTR strict Mendelian rate (%%)")
    parser.add_argument("--mendelian-1motif", type=float, default=None,
                        help="HaploTR ±1motif Mendelian rate (%%)")
    ...
    ht_m = compute_haplotr_metrics(ht_pairs)
    lt_m = compute_tool_metrics(lt_pairs)
    trgt_m = compute_tool_metrics(trgt_pairs)

    logger.info("Matched: HaploTR=%d, LongTR=%d, TRGT=%d",
    ...
    data = []
    for name, m in [("HaploTR v4", ht_m), ("LongTR", lt_m), ("TRGT", trgt_m)]:
        data.append([
            name, f"{m['n']:,d}",
